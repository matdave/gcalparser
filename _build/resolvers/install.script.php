...
/**
 * Description: Resolver script for gcalparser package
 * @package gcalparser
 * @subpackage build
 */
    ...
    case xPDOTransport::ACTION_INSTALL:
        ...
        foreach ($settings as $key) {
            if (isset($options[$key])) {
                $setting = $object->xpdo->getObject('modSystemSetting', array('key' => 'gcalparser.' . $key));
                if ($setting != null) {
                    $setting->set('value', $options[$key]);
                    $setting->save();
                } else {
                    $object->xpdo->log(xPDO::LOG_LEVEL_ERROR, '[gcalparser] ' . $key . ' setting could not be found, so the setting could not be changed.');
                }
            }
        }
